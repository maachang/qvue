...
var dialog = (function() {
  ...
  var _alert = function(text, call) {
    ...
    return "<div id='" + _ALERT_ID + "' style='z-index:"+DIALOG_ZINDEX+";position:absolute;left:"+p.left+"px;top:"+p.top+"px;"+
  ...
  // アラートイベントをセット.
  var _alertEvent = function() {
    setTimeout(function() {
      var em = _$(_ALERT_ID);
      if(em) {
        addEvent(em, "click", function() {_closeDialog("alert", "");return true;});
      }
    }, 50);
  }

  // 確認ダイアログ表示.
  var _confirm = function(text, call, opt) {
    if(typeof(call) == "function") {
      _DIALOG_CALL[0] = call;
    } else {
      _DIALOG_CALL[0] = null;
    ...
    // 大枠をセット.
    ret = "<div id='" + _COMFIRM_ID + "' style='z-index:"+DIALOG_ZINDEX+";position:absolute;left:"+p.left+"px;top:"+p.top+"px;"+
      "width:"+p.width+"px;height:"+p.height+"px;border-radius:"+p.radius+"px;"+
      "background:#ffffff;color:#000000;border: solid 2px #efefef;"+
      _BACK_DIALOG_SHADOW +
      " ontouchmove='return true;'>";
    // テキスト表示枠をセット.
    ret += "<div id='" + _COMFIRM_ID + "_inner' style='z-index:"+(DIALOG_ZINDEX+1)+";left:"+(p.left-(pd>>1))+"px;top:"+(p.top-(pd>>1))+"px;"+
    ...
    // yesボタン.
    ret += "<a href='javascript:void(0);' id='"+ _COMFIRM_ID + "_yesButton' ";
    if(ycl) {
      ret += "class='"+ycl+"' ";
    }
    ...
    // noボタン.
    ret += "<a href='javascript:void(0);' id='"+ _COMFIRM_ID + "_noButton' ";
    if(ncl) {
      ret += "class='"+ncl+"' ";
    }
  ...
  var _confirmEvent = function() {
    setTimeout(function() {
      var em = _$(_COMFIRM_ID+"_yesButton");
      if(em) {
        addEvent(em, _CLICK, function() {_closeDialog("confirm", true);return true;});
      }
      em = _$(_COMFIRM_ID+"_noButton");
      if(em) {
        addEvent(em, _CLICK, function() {_closeDialog("confirm", false);return true;});
      }
    }, 50);
  }

  // ダイアログクローズ.
  var _closeDialog = function(type, yes, call) {
    if(typeof(call) != "function") {
        call = _DIALOG_CALL[0];
    }
    _DIALOG_CALL[0] = null;
    ...
    if(em) {
      executionTime(_CLOSE_DIALOG_TIMING, function(dialogType,yesFlag,callFunc) {
        ...
        var targetEm = null;
        if(dialogType == "confirm") {
          targetEm = _$(_COMFIRM_ID);
        } else {
          targetEm = _$(_ALERT_ID);
        }
  ...
  o.alert = function(text, err, call, id) {
    if(!id) {
      id = _QVUE_DIALOG_ID;
    }
    var em = _$(id);
    if(em) {
      em.innerHTML =  fillAlpha.start(DIALOG_ZINDEX-1) + _alert(text,call) + "\n<div>";
      _alertEvent();
  ...
  // alertを非表示.
  o.closeAlert = function(call) {
    _closeDialog("alert", "", call);
  }
  ...
  // confirmを表示.
  o.confirm = function(text, call, opt, id) {
    if(!id) {
      id = _QVUE_DIALOG_ID;
    }
    var margeOpt = {};
    ...
    var em = _$(id);
    if(em) {
      em.innerHTML =  fillAlpha.start(DIALOG_ZINDEX-1) + _confirm(text,call,margeOpt) + "\n<div>";
      _confirmEvent();
      return true;
    }
    return false;
  ...
  // confirmを非表示.
  o.closeConfirm = function(yes, call) {
    _closeDialog("confirm", (yes == true), call);
  }
...
var _loadPage = function(id, url, params, headers, call) {
  ...
  phttpc.get({url: path + ".html", body: params, headers: headers})
    ...
    .then(function(result) {
      ...
      if(p != -1) {
        var pp = notCoteIndex(result.body, QVUE_END_TAG, p + QVUE_END_TAG_LEN);
      ...
      phttpc.get({url: path + ".js", body: params, headers: headers})
        ...
        .then(function(result) {
          // 取得したjsをセット.
          return call(html, _jsParams(result.body, params));
        })
        .catch(function() {
          // jsファイルが存在しないので、ダミーをセットする.
          return call(html,
            _jsParams("\"use strict\";\n(function(_g) {var app = qvue('<<ID>>').execute()})(this);", params));
        });
    })
    .catch(function() {
      // htmlが存在しないので、空をセット.
      phttpc.get({url: path + ".js", body: params, headers: headers})
        ...
        .then(function(result) {
          // 取得したjsをセット.
          return call("", _jsParams(result.body, params));
        })
        .catch(function() {
          // エラーを設定.
  ...
  _loadPage(id, src, params, {}, function(html, js) {
    ...
    try {
      ...
      if(em) {
        ...
        notEnd = true;
        _errorPage(id, 500, "エラーが発生しました:" + "id: " + id + " は存在しません");
      }
    } catch(e) {
      notEnd = true;
      _errorPage(id, 500, "エラーが発生しました:" + e);
      console.trace(e);
    } finally {
      if(!notEnd) {
        setTimeout(function() {
...
// ページ移動.
o.movePage = function(title, src, params, id) {
  if(!id || id == "") {
    id = _QVUE_BODY_ID;
  }
  if(!title) {
...
// alert処理.
o.alert = function(text, err, call, id) {
  dialog.alert(text, err, call, id);
}

// confirm処理.
o.confirm = function(text, call, opt, id) {
  dialog.confirm(text, call, opt, id);
}

// qvue初期化処理.
o.init = vueObject.init;

// alert処理を乗せかえる.
_g.alert = function(text, err, call, id) {
  dialog.alert(text, err, call, id);
}
